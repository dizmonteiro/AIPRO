...
public class MainContainer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			int limit_bikes = 5; // Limit number of Taxis
			int limit_users = 100; // Limit number of Customers

			// Start agents Bikes!
			for (int n = 0; n < limit_bikes; n++) {
				a.startAgentInPlatform("Bike" + n, "Agents.Bike");
			}
			...
			for (int n = 0; n < limit_users; n++) {
				
				// Sleep 1 second for each x customer agents added
				if (n % 10 == 0) {
					Thread.sleep(1000);
				}
				a.startAgentInPlatform("Customer" + n, "Agents.UserAgent");
			}
